...
/// and `cargo vendor` output under `…/vendor/…`. Backslashes are normalized to
/// forward slashes first so the same check works on Windows.
fn is_dev_source_tree(manifest_dir: &Path) -> bool {
    let normalized = manifest_dir.to_string_lossy().replace('\\', "/");
    const NON_SOURCE_MARKERS: [&str; 4] = [
        "/registry/src/",
        "/registry/cache/",
        "/git/checkouts/",
        "/vendor/",

...
- **`src/actions/migrate.rs`** — Cross-method install cleanup, exposed as the **hidden** `nd300 migrate-cleanup` subcommand (`#[command(hide = true)]`). Invoked by the four Windows installers (and the silent self-update path) to remove a shadowing older Cargo pair, limited other-edition files, and retired update images. It reuses `uninstall_path_files_only` / `OUR_BINARIES` and the updater's ownership/path helpers; it does not re-implement deletion. Publicly usable flags remain `--cargo-copy`, `--other-edition`, `--quiet`, `--dry-run`, `--json`, `--user-profile <path>`, `--cargo-home <path>`; installers additionally pass hidden validated `--install-origin <msi-global|msi-corporate|exe-global|exe-corporate>` and `--retired-update`. Retired cleanup accepts only versioned `nd300.update-old-*` / `speedqx.update-old-*` siblings in the running install directory plus the fixed pre-v3.1 Global MSI directory. Hard guarantees (unit-tested): never cargo/rustup/PATH/ARP/receipts/the shared marker/`~/Downloads`; never escalates. **Always exits 0** except on a true internal error — cleanup is advisory and must never fail an active installer transaction.

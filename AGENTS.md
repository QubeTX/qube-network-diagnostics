...
On Windows, the installer marker/dispatch matrix is unchanged. Cargo is bounded and post-install verification checks both explicit Cargo-bin binaries at the exact version; no updater path runs `rustup update`. JSON retains the Windows-only top-level `install_origin` field and `msi_global`/`msi_corporate`/`exe_global`/`exe_corporate` strategy IDs. Windows-only checksum/registry helpers remain cfg-gated so macOS/Linux `-D warnings` stays strict.

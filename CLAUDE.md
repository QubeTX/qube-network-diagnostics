...
- **`crates-publish.yml`** — publishes the `nd300` crate, fired via `workflow_run` after `CI` succeeds on a `main` push. Idempotent (skips a version already on crates.io). This is the ONLY place the crate is published; it pins Rust 1.97.0 to match CI/release validation.

...
4. Build ordered strategies: bounded Cargo first; native exact-tag Unix archive fallback; unchanged matching Windows MSI/EXE or PowerShell/pwsh paths.
5. Cargo runs as the validated invoking user under a deadline. Never run `rustup update`.
6. Verify explicit `~/.cargo/bin/nd300` AND `speedqx` at the exact expected version before any known legacy cleanup.
7. Unix fallback downloads the exact target `.tar.xz` + required sidecar under time/size limits; verifies SHA-256; extracts only two allowlisted regular files; rejects absolute/traversal/link/duplicate/unexpected layouts; executes both staged versions.
8. macOS additionally verifies the pinned Developer ID leaf, Team ID, per-binary identifier, hardened runtime, and timestamp. Transactionally swap both with backups and rollback both on any failure.
...
- **Cargo**: bare Cargo cannot run project-specific cleanup. ND300 cleans only a validated legacy origin after both explicit Cargo-bin versions match.
